...
func (h5 *HDF5) readSuperblock() {
	...
	warnAssert(uint64(h5.fileSize) == eofAddr,
...
func readNullTerminatedName(padding int, properties []byte) (string, int) {
	bf := newResetReaderFromBytes(properties)
	var name []byte
	nullFound := false
	plen := 0
	for !nullFound {
...
func checkZeroes(bf io.Reader, len int) {
	padBytesCheck(bf, len, false /*don't round*/, true /*don't warn only*/)
}
...
func (h5 *HDF5) printDatatype(obj *object, b []byte, bf remReader, objCount int64, attr *attribute, isCompound bool) (int, int64) {
	assert(len(b) >= 8, "short data")
	...
	properties := b[8:]
	logger.Infof("* datatype=0x%02x length=%d dtlength=%d dtversion=%d class=%s flags=%s",
		b, len(b), dtlength,
	...
	case typeFixedPoint:
		...
		if len(properties) == 0 {
			...
			bff := newResetReaderFromBytes(properties)
	...
	case typeFloatingPoint:
		...
		logger.Info("data len", bf.Rem())
		if bf.Rem() < int64(dtlength) {
			logger.Info("data short", dtlength, bf.Rem())
			break
		}
		attr.value = h5.getDataAttr(bf, *attr)
		dlen += int64(dtlength) * objCount
	...
	case typeOpaque:
		logger.Info("opaque, not fully working", len(properties))
		if len(properties) == 0 {
	...
	case typeCompound: // compound
		...
		for i := 0; i < int(nmembers); i++ {
			...
			bff := newResetReaderFromBytes(properties[poff:])
			var byteOffset uint32
			switch dtversion {
			case 1, 2:
				byteOffset = read32(bff)
			...
			logger.Infof("%d compound before: len(prop) = %d len(data) = %d", i,
				len(properties[poff:]), rem)
			...
			logger.Infof("%d compound after: len(prop) = %d len(data) = %d", i, len(properties[poff:])-thisb, rem-thisd)
			blen += thisb
			poff += thisb
			dlen += thisd
	...
	case typeReference:
		...
		bf := newResetReader(bf, int64(dtlength))
		addr := read64(bf)
		dlen += 8
	...
	case typeEnumerated:
		...
		bff := newResetReaderFromBytes(properties)
		for i := uint32(0); i < numberOfMembers; i++ {
			values[i] = h5.getDataAttr(bff, enumAttr)
...
func (h5 *HDF5) readAttribute(obj *object, obf io.Reader, size uint16, creationOrder uint64) {
	...
	if !shared {
		_, _ = h5.printDatatype(obj, dtb, bf, count, attr, false /*isCompound*/)
	} else {
...
func (h5 *HDF5) readBTreeInternal(parent *object, bta uint64, numRec uint64, recordSize uint16, depth uint16, nodeSize uint32) {
	...
	for i := uint64(0); i <= nr; i++ {
		...
		assert(maxNumberOfRecords < 256, "can't handle this") // TODO: support bigguer maxes
		cnr := read8(bf)
...
func allocInts(bf io.Reader, dimLengths []uint64, endian binary.ByteOrder, signed bool) interface{} {
	if len(dimLengths) == 0 {
		...
		err := binary.Read(bf, endian, &value)
		thrower.ThrowIfError(err)
		logger.Info("fixed32value=", value)
		if signed {
			return int32(value)
		}
		return value
...
func allocFloats(bf io.Reader, dimLengths []uint64, endian binary.ByteOrder) interface{} {
	if len(dimLengths) == 0 {
		...
		err := binary.Read(bf, endian, &value)
		thrower.ThrowIfError(err)
		logger.Info("32value=", value, "rem=", bf.(remReader).Rem())
		return value
	}
...
func padBytes(bf io.Reader, pad32 int) {
	padBytesCheck(bf, pad32, true /*round*/, false /*don't warn only*/)
}
